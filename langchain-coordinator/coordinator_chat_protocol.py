#!/usr/bin/env python3
"""
LangChain Coordination Agent - Chat Protocol Version
Works with Fetch.ai agents using Chat Protocol on Agentverse
"""
import os
import asyncio
...
Return ONLY valid JSON, no explanation."""
    ...
    async def send_to_need_agent_via_chat(self, message: str) -> bool:
        """Send message to need agent using Chat Protocol via DeltaV or direct message"""
        ...
        try:
            # Option 1: Via DeltaV Chat API (if available)
            # For Agentverse deployed agents, you would use the DeltaV chat endpoint
            
            # Option 2: Store in Claude service for the need agent to poll
            async with httpx.AsyncClient(timeout=30) as http_client:
                response = await http_client.post(
                    f"{CLAUDE_SERVICE_URL}/api/uagent/update",
                    json={
                        "request_id": f"COORD-{int(asyncio.get_event_loop().time())}",
                        "agent_id": "langchain_coordinator",
                        "type": "need_request",
                        "data": {
                            "message": message,
                            "need_agent_address": NEED_AGENT_ADDRESS,
                            "supply_agent_address": SUPPLY_AGENT_ADDRESS,
                ...
                if response.status_code == 200:
                    print(f"✅ Message stored for need agent to process")
                    return True
                else:
                    print(f"⚠️  Storage returned: {response.status_code}")
                    return False
    ...
    async def process_chat_message(self, user_message: str, request_id: str = None) -> Dict[str, Any]:
        ...
        # Step 2: Create structured request for Claude service
        request_data = {
            "input": user_message,
            "source": "chat",
            "analysis": analysis,
            "need_agent": NEED_AGENT_ADDRESS,
            "supply_agent": SUPPLY_AGENT_ADDRESS
        }
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as http_client:
                ...
                if response.status_code == 200:
                    ...
                    # Step 4: Format message for need agent
                    need_agent_message = self.format_message_for_need_agent(analysis)
                    
                    # Step 5: Send to need agent via Chat Protocol
                    await self.send_to_need_agent_via_chat(need_agent_message)
                    ...
                    return {
                        ...
                        "message": f"✅ Request received! Priority: {analysis.get('priority', 'medium').upper()}. "
                                  f"Coordinating with supply agents via Chat Protocol..."
                    }
                else:
                    print(f"⚠️  Claude service returned: {response.status_code}")
                    # Still try to forward to need agent
                    fallback_id = request_id or f"REQ-{int(asyncio.get_event_loop().time())}"
                    need_agent_message = self.format_message_for_need_agent(analysis)
                    await self.send_to_need_agent_via_chat(need_agent_message)
                    ...
                    return {
                        ...
                        "analysis": analysis,
                        "message": f"Request forwarded (Priority: {analysis.get('priority', 'medium').upper()})"
                    }
    ...
    async def poll_and_coordinate(self):
        ...
        print(f"   Protocol: Chat Protocol (Agentverse)")
        print(f"   Polling interval: 5 seconds\n")
        
        while True:
            try:
                # Poll Claude service for new requests
                async with httpx.AsyncClient(timeout=10) as http_client:
                    # Check for pending requests that need coordination
                        ...
                        for req in requests:
                            ...
                            if req_id not in self.processed_requests:
                                ...
                                if original_input:
                                    ...
                                    print(f"✅ Processed request: {req_id}")
                                else:
                                    # Old request without raw_input, skip it
                                    print(f"⏭️  Skipping old request (no input): {req_id}")
                                    self.processed_requests.add(req_id)
                    else:
                ...
                print(f"⚠️  Cannot connect to Claude service at {CLAUDE_SERVICE_URL}")
                print(f"   Make sure the service is running: cd agentaid-claude-service && node server.js")
            except Exception as e:
...
async def main():
    """Main entry point"""
    print("=" * 70)
    print("🧠 LangChain Coordination Agent - CHAT PROTOCOL")
    print("=" * 70)
    ...
    # Test with a sample message if TEST_MODE is enabled
